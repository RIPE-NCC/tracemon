...
net.webrobotics.TreeMap = function (comparator, options) {
    ...
    this.elements = [];
    this.comparator = comparator || function (key1, key2) {
        if (key1 < key2) {
            return -1;
        } else {
...
net.webrobotics.TreeMap.prototype.nearest = function (key, after, included) {
    var elements, included, position, element;
    included = (included != null) ? included : true;
    position = this.positionOf(key);
...
net.webrobotics.TreeMap.prototype.median = function(){
    var size, even, half;
    ...
    if (even){
        return [this.at(half), this.at(half + 1)];
    }else{
        return [this.at(half)];
    }
